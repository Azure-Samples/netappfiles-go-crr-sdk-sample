...
func CreateAnfVolume(ctx context.Context, location, resourceGroupName, accountName, poolName, volumeName, serviceLevel, subnetID, snapshotID string, protocolTypes []string, volumeUsageQuota int64, unixReadOnly, unixReadWrite bool, tags map[string]*string, dataProtectionObject armnetapp.VolumePropertiesDataProtection) (*armnetapp.Volume, error) {
	...
	emptyDataProtection := armnetapp.VolumePropertiesDataProtection{}
...
func WaitForMirrorState(ctx context.Context, volumeID string, anticipatedMirrorState armnetapp.MirrorState, intervalInSec int, retries int) error {
	var err error
	var currentReplicationStatus armnetapp.VolumesClientReplicationStatusResponse
	...
	for i := 0; i < retries; i++ {
		...
		currentReplicationStatus, err = client.ReplicationStatus(
			ctx,
